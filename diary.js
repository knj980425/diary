...
function renderAllEntries() {
  ...
  diary.forEach(function (d) {
    const ret = renderEntry(d);
    diaryList.appendChild(ret);
  });
...
function updateEntry(id, updates) {
  ...
  const upentry = diary.find(function (d) {
    if (d.id === id) {
      return true;
    }
  });
  upentry && Object.assign(upentry, updates);
}
...
function fillFormForEdit(id) {
  ...
  const findDiary = diary.find(function (d) {
    return d.id === id;
  });
  if (!findDiary) {
    return;
  }
  titleInput.value = findDiary.title;
  contentInput.value = findDiary.content;
  moodSelect.value = findDiary.mood;
  editingId = id;
...
function deleteEntry(id) {
  ...
  const diaryIndex = diary.findIndex(function (d) {
    return d.id === id;
  });
  if (diaryIndex !== -1) {
...
function getDiaryStats() {
  ...
  const diarystats = { total: diary.length, moods: {} };
  diary.forEach(function (d) {
    if (diarystats.moods[d.mood]) {
      diarystats.moods[d.mood]++;
    } else {
      diarystats.moods[d.mood] = 1;
    }
  });
  return diarystats;
}
...
function renderStats() {
  ...
  const statTotal = document.createElement("h3");
  statTotal.textContent = `전체 일기: ${stats.total}개`;
  statTotal.className = "stat-total";
  statsGrid.appendChild(statTotal);
  statsContainer.appendChild(statsGrid);
  Object.entries(stats.moods).forEach(function ([m]) {
    const statItem = document.createElement("span");
    statItem.className = "stat-item";
    statItem.textContent = MOOD_EMOJIS[m];
...
const getEntriesByKeyword = function (keyword) {
  return diary.filter(function (d) {
    return d.title.includes(keyword) || d.content.includes(keyword);
  });
};

// function renderAllEntries(mood) {
//   diary.filter(function (d) {
//     return d.mood === mood;
//   });
// }
